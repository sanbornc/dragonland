---
name: Party Wagon
aliases: [the wagon]
type: item
tags: [party-asset, magic-item, storage]
first-learned: session 01
---

## What we know
- A **party-owned wagon** enchanted with **Bag of Holding–like** properties — it
  holds far more than its physical size should allow (extradimensional storage).
- **Convention:** any **unclaimed loot** defaults to the wagon. If an item isn't
  being carried by a specific character, assume it is stored here.

## Stored here by default (unclaimed loot)
- Ringmaster's Crimson Cane — [Ringmaster's Crimson Cane](ringmasters-crimson-cane.md)
- Shadow-Stained Cloak — [Shadow-Stained Cloak](shadow-stained-cloak.md)
- Calliope Shard — [Calliope Shard](calliope-shard.md)
- Ringmaster's Final Ticket — [Ringmaster's Final Ticket](ringmasters-final-ticket.md)

*(Claimed instead: Renob carries the magic bone dice. The Wolfcall Whistle is a
shared party item — [Wolfcall Whistle](wolfcall-whistle.md).)*

## Session 02 — buried-archive haul (party inventory)
All gold and moderate-value loot from the tower/archive is pooled here. The
**live party inventory is maintained as custom items on Boilerdang's D&D Beyond
sheet**; this list mirrors it.

- **Coin (party gold):** **1,356 gp, 224 sp, 119 cp** — the gold recovered in the archive plus the proceeds from selling the bulk weapons and the 2 moonstones.
- **Gems & valuables:** **Silver Holy Symbol of Paladine.** *(The 2 Moonstones were sold.)*
- **Magic / utility items:** **Black Banner Signet Ring** ([Black Banner Signet Ring](black-banner-signet-ring.md)); **Solamnic Officer's Compass** ([Solamnic Officer's Compass](solamnic-officers-compass.md)).
- **Consumables & supplies:** 2 Potions of Healing, Medical Kit + bandages, ~4 days' rations and food, 4 lanterns, 6 flasks of oil, 2 bundles of torches.
- **Bulk weapons — SOLD (session 02 aftermath):** the 9 shortbows, 4 serviceable scimitars, and 3 daggers (with 120 arrows) were sold for gold added to the party pool. *(Verified gone from Boilerdang's live sheet.)*
- **Documents (kept for the record):** Dragon Prophecy parchment, Knights of Solamnia sigil sheet, Captain Varek's papers, Black Banner guard orders, and the goblin-camp finds (pay chit, crude map, dig orders).
- *Goblin trinkets (mostly worthless): chicken-bone necklace, three silver buttons, a wolf-head whistle, animal teeth, a "lucky" gold-painted copper, a pouch of colorful stones.*

*Claimed by a character (not in the party pool): **Vaelor** carries the **Oathkeeper's Blade** ([Oathkeeper's Blade](oathkeepers-blade.md)).*

## Session 03 — Solace roadblock haul (party inventory)
Recovered from the wiped-out [Seeker](../factions/the-seekers.md) roadblock crew at
Solace (Session 03). Pooled here by default. **Boilerdang's live D&D Beyond sheet
should be updated to match.**

- **Coin added:** 16 gp, 47 sp, 31 cp (on the bodies) + 11 gp, 22 sp (lock box) =
  **+27 gp, +69 sp, +31 cp.** New party total: **1,383 gp, 293 sp, 150 cp.**
- **Notable / keep:**
  - **Brass authority token** marked with **High Theocrat Hederick's office** — a
    potential pass/forgery for checkpoints. ([The Seekers](../factions/the-seekers.md))
  - **Seeker-marked cloak clasp shaped like a sunburst** — the Seekers' insignia;
    useful as evidence or disguise.
  - **1 folded field order** from **Fewmaster Toede** to Voss — see
    [Seeker Field Order — Toede to Voss](../documents/seeker-field-order-toede.md).
  - **Bone signal whistle**; **2 hooded checkpoint lanterns.**
  - **Serviceable rapier** taken from **[Pellan Voss](../npcs/pellan-voss.md)**.
  - **Lock-box holy items (confiscated by the Seekers):** 4 holy trinkets; a cheap
    **wooden symbol of [Mishakal](../lore/mishakal.md)** labeled "false god token";
    a **broken Solamnic kingfisher brooch** ([Knights of Solamnia](../factions/knights-of-solamnia.md)).
  - **2 healing herb bundles**; **1 sealed bottle of decent brandy.**
- **Bulk gear — USED to stage the scene (NOT in inventory):** the 7 crude scimitars,
  7 shortbows, 83 black-feathered arrows, 5 dented small shields, 7 sets of poor
  goblin leather armor, 2 hunting snares, 50 ft. rough rope, and 3 days of (bad)
  trail rations were **expended/left at the roadblock** to pose the dead crew as
  having died in an **internal dispute.** The party passed a **Deception check**, so
  [Fewmaster Toede](../npcs/fewmaster-toede.md) should believe it was infighting —
  not the party. (Session 03)

## Open questions
- Exact capacity and any limits (described only as "similar to a bag of holding").
- Does the wagon have a name?

## See also
- [Dragonland — Campaign Overview](../../OVERVIEW.md) · [Session 01 — Crimson Circus Crossover (Bridge Session)](../../sessions/01-2026-06-02-crimson-circus-crossover.md)
