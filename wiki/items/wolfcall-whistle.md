---
name: Wolfcall Whistle
aliases: []
type: item
tags: [magic-item, circus, lycanthropy]
first-learned: session 01
---

## What we know
- A whistle the party carries, brought forward from the Crimson Circus arc.
  (`sessions/01-2026-06-02-crimson-circus-crossover.md`)
- Gained a **Bloodmoon Charm** upgrade during the bridge session, adding 1/day
  **Hunter's Lure**: one beast, wolf, or werewolf within 60 ft that can hear the
  whistle makes a DC 14 Wisdom save or must use its next movement to move toward
  the whistle-blower (or a chosen point within 30 ft of them). It redirects the
  hunt but does not fully control the creature. (session 01)

## Open questions
- What are the whistle's base abilities (before the Bloodmoon upgrade)? Who
  carries it?

## See also
- `wiki/lore/the-crimson-circus.md`
